 ...
 $url;
  ...
  if($loginAttempts > 2){														//exceeded logins
        ...
        if($logincheck->doCheck() == true){
        	...
        	if($logincheck->profile->getType()==="admin"){header('Location: browseC.php');} else {header('Location: settingsC.php');}
            ...
            settype($foo, "integer");
            $foo.=$loginAttempts;
            $errorMessage = "Error: Unrecognized Username or Password, You have " . (3-$foo) . " Login Attempts Left";
